import debasisImage from "../assets/images/about/debasis.webp";
import shankarImage from "../assets/images/about/shankar.webp";
import man from "../assets/images/about/man.webp";
import sandeep from "../assets/images/about/sandeep.webp";
import pranjal from "../assets/images/about/pranjal.webp";
import pradeep from "../assets/images/about/pradeep.webp";
import archan from "../assets/images/about/archan.webp";

export const teamData = [
  {
    id: 1,
    name: "Debasis Panda",
    designation: "Founder Director, CEO",
    image: debasisImage,
    linkedin: "https://www.linkedin.com/in/debasis-panda-a87a245/",
    desc: "Mr. Debasis Panda, Founder Director of Shiretechnik Solutions Private Limited and SANDEB TECH Private Limited, has two decades of professional experience across various roles. His expertise includes extensive work as a software developer for thermal management software, as well as significant experience in HVAC CFD, design solutions, and fire and smoke management. Mr. Panda has successfully completed numerous carpark and fire safety analysis projects both in India and abroad. He was instrumental in establishing the association with Thunderhead Engineering and continues to lead software service sales for the Indian and south Asia market at Shiretechnik. Mr. Panda holds a postgraduate degree and previously worked with the Symbiose group at IRISA in Rennes, France"
  },
  {
    id: 2,
    name: "Shankar S",
    designation: "Director",
    image: shankarImage,
    linkedin: "https://www.linkedin.com/in/shankar-s-ba388533/",
    desc: "Mr. Shankar has more than 25 years of industry experience in thermal management of electronic systems, electro-mechanical design, thermal CFD code development solutions & fire protection engineering simulations. He worked with the leading development teams in public & private sectors and keeps proven-track of records in electronic product design, project management, research & development, high and low volume manufacturing & reliability engineering in telecom, automobile, military & defense, aerospace, space & consumer industries and his engineering experience also spans in combustion technology, fire & safety analysis and HVAC CFD simulations. Mr. Shankar holds a Master’s degree in Thermal Engineering from IIT Madras, Chennai and two Bachelor degrees in Mechanical Engineering from Institute of Engineers, Kolkata and in Mathematics from Madras University, Chennai. He is a certified Reliability Professional from Ministry of Communication and Information Technology. Govt of India. He authored several papers on thermal modelling in CFD environments and electronic product engineering and serving as a technical review lead in international conferences."
  },
  {
    id: 3,
    name: "Archan Chatterjee",
    designation: "Director",
    image: archan,
    linkedin: "",
    desc: "Archan, having more than 15 years experience in the field of engineering sales and marketing. He has International marketing and business development experience. Expertise in new product promotions, sales, business strategy making."
  },
  {
    id: 4,
    name: "Sandeep Nandi",
    designation: "Principal Consultant IT and AI",
    image: sandeep,
    linkedin: "https://www.linkedin.com/in/sandeep-nandi-06409310/",
    desc: "Sandeep has more than twelve years of professional experience in CAE Software development domain and hardware product engineering in private sectors. He was the part of FloTHERM development team and also has worked for Dassault Systems, ESI etc. He holds Master’s and Bachelor’s degrees in Mechanical Engineering from Indian Institute of Technology (IIT), Kharagpur. He has deep expertise in CFD simulation tools. He is also co-founder of SANDEBTech Pvt. Ltd. Track Record – Dassault Systems, ESI, Flomerics India Pvt Ltd sportskeeda.com, policymantra.com."
  },
  {
    id: 5,
    name: "Pranjal Mishra",
    designation: "CAE Engineer",
    image: pranjal,
    linkedin: "https://www.linkedin.com/in/pranjal-m11/",
    desc: "Pranjal Mishra is a skilled Computer-Aided Engineering (CAE) Engineer specializing in simulation, analysis, and optimization of engineering designs using advanced computational tools. As a CAE Engineer, he focuses on: Performing finite element analysis (FEA) and simulations, Analyzing structural, thermal, and fluid behavior of components, Improving product design through virtual testing, Reducing development cost and time by minimizing physical prototypes, Collaborating with design and manufacturing teams."
  },
  {
    id: 6,
    name: "Pradeep Kunta",
    designation: "Service Engineer",
    image: pradeep,
    linkedin: "https://www.linkedin.com/in/pradeep-reddy-883b6715b/",
    desc: "Pradeep Kunta is a dedicated and skilled Service Engineer with expertise in installation, maintenance, and troubleshooting of technical systems and equipment. He plays a key role in ensuring smooth operations and customer satisfaction through timely and efficient service support. Responsibilities include: Installing and configuring equipment or software systems, Diagnosing and resolving technical issues, Performing routine maintenance and inspections, Providing on-site and remote technical support, Coordinating with clients to understand and resolve service requirements, Ensuring minimal downtime and high system performance."
  },
  {
    id: 7,
    name: "Sarang Shidore",
    designation: "Advisory Board Member",
    image: man,
    linkedin: "https://www.linkedin.com/in/sandeep-nandi-06409310/", // Note: This is the same LinkedIn as Sandeep, likely a placeholder
    desc: "Sarang Shidore is an expert in semiconductor thermal phenomena and corporate strategy and product planning. He received his Bachelor’s degree from the Indian Institute of Technology, Madras, and his Master’s degree from the University of Texas, Austin. Since 1995, he worked at Flomerics Inc., at Austin, TX, in various roles in engineering and product management with a special focus on package-level thermal modeling and analysis, a field in which he authored several papers and articles, and also developed two international standards as Vice-Chair of the JC15 committee of JEDEC, the international standards body for the semiconductor industry. At Flomerics, he had worldwide product responsibilities – from roadmaps to rollout. He Conceptualized and created two web-based software products in semiconductor thermal analysis catering to several Fortune 500 companies such as Texas Instruments, Intel, ST Microelectronics, etc. The product made a quantum leap in Flomerics’ offerings to the semiconductor industry. He was also key in establishing the Flomerics Development Centre in Bangalore. In addition, he worked for Mentor Graphics as Product Marketing Manager and for several years as a consultant for various organizations. He is currently a visiting scholar at the Lyndon B. Johnson School of Public Affairs at the University of Texas at Austin, focused on energy and climate policy and engaged in consulting in strategy and scenario planning."
  }
];